...
it("should handle mismatched user ID in request and params", async () => {
    const invalidMockReq = { ...mockReq, user: { id: "differentUserID" } };
  
    await updateUser(invalidMockReq, mockRes);
  
    expect(require("../models/user.model.js").default.findByIdAndUpdate).not.toHaveBeenCalled();
    expect(mockRes.status).toHaveBeenCalledWith(403);
    expect(mockRes.json).toHaveBeenCalledWith("Unauthorized access");
  });
